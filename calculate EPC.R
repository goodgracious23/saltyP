...
ggsave("epc_season_vs_bottomSRP.pdf",
  ggarrange(gg_epc_season, gg_epc_minus_hypoSRP, nrow = 2), 
...
gg_epc_chloride = 
  ...
  geom_point(size = 4, alpha = 0.7, aes(color = pond, shape = season)) + 
  scale_color_manual(
...
gg_epc_srp = 
  ...
  facet_wrap(~InitialCl_label, nrow = 1) +
  theme_bw() + 
  ylab("EPC") + xlab("SRP at Time of Sampling (mg/L)")
...
gg_epc_pH = 
  ...
  facet_wrap(~InitialCl_label, nrow = 1) +
  theme_bw() + 
  ylab("EPC") + xlab("pH at Time of Sampling (mg/L)")
